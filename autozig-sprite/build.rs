//! Build script for autozig-sprite
//! Compiles Zig sprite rendering code
...
fn main() {
    ...
    if let Ok(profile) = std::env::var("PROFILE") {
        ...
        for deps_dir in dirs_to_clean {
            if deps_dir.exists() {
                // Remove all libautozig_sprite-*.a and autozig_sprite-*.a archives
                if let Ok(entries) = std::fs::read_dir(&deps_dir) {
                    for entry in entries.flatten() {
                        let path = entry.path();
                        if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                            // Clean ONLY this crate's artifacts
                            let should_remove = (name.starts_with("libautozig_sprite-") ||
                                                name.starts_with("autozig_sprite-")) &&
    ...
    println!("cargo:warning=Using MODULAR_BUILDZIG compilation mode for autozig-sprite");
