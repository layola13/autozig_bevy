//! Build script for autozig-ui
//! Compiles Zig UI rendering code
...
fn main() {
    ...
    if let Ok(profile) = std::env::var("PROFILE") {
        ...
        for deps_dir in dirs_to_clean {
            if deps_dir.exists() {
                // Remove all libautozig_ui-*.a and autozig_ui-*.a archives
                if let Ok(entries) = std::fs::read_dir(&deps_dir) {
                    for entry in entries.flatten() {
                        let path = entry.path();
                        if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                            // Clean ONLY this crate's artifacts
                            let should_remove = (name.starts_with("libautozig_ui-") ||
                                                name.starts_with("autozig_ui-")) &&
    ...
    println!("cargo:warning=Using MODULAR_BUILDZIG compilation mode for autozig-ui");
