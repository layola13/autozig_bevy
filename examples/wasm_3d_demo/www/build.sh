#!/bin/bash
# AutoZig WASM 3D Demo - 构建脚本

set -e

echo "🚀 AutoZig WASM 3D Demo 构建"
echo "===================================="
echo ""
...
echo ""
echo "✅ 构建完成！"
echo "   输出: target/wasm64-unknown-unknown/release/autozig_wasm_3d_demo.wasm"

# 复制wasm文件到www目录
echo ""
echo "📦 准备Web部署文件..."
mkdir -p www
cp ../target/wasm64-unknown-unknown/release/autozig_wasm_3d_demo.wasm www/

# 复制 autozig 自动生成的 TypeScript 绑定文件
BUILD_OUT_DIR=$(find ../target/wasm64-unknown-unknown/release/build/autozig-wasm-3d-demo-*/out -maxdepth 0 2>/dev/null | head -1)
